...
if (headerSearch) {
    headerSearch.addEventListener('search', async (e) => {
        ...
        renderLoadingRow();
        const res = await get(API_ENDPOINTS.SEARCH_EMPLOYEE(q));
        if (res.success && res.data.status) {
            renderRows(res.data.data);
        } else {
            tbody.innerHTML = `<tr><td colspan="6" class="px-6 py-10 text-center text-sm text-red-500">${res.data?.message || res.error || 'Failed to search employees.'}</td></tr>`;
            toast.show(res.data?.message || res.error || 'Failed to search employees.', 'error');
        }
    });
}
